...
-- date included 
SELECT Location,population,date, Max(total_cases) as highest_infectionCount,(CAST(Max(total_cases) AS float) / CAST(population AS float)) * 100 AS Percentage_of_infected 
FROM PortfolioProject..CovidDeaths$
